    ...
    for k in ks:
        ...
        if 'GA' in algorithms:
            ...
            for eta in etas:
                ...
                if sum(OL.int_rewards) > sum(rewards):
                    ...
                    results[k]['frac_rewards_GA'] = OL.frac_rewards
                    results[k]['int_rewards_GA'] = OL.int_rewards
                    results[k]['frac_sol_GA'] = OL.xs[-1]
                    results[k]['int_sol_GA'] = OL.xs_int[-1]
                    results[k]['eta_GA'] = eta
        ...
        if 'one-stage GA' in algorithms:
            ...
            for eta in etas:
                ...
                if sum(OL.int_rewards) > sum(rewards):
                    rewards = OL.int_rewards
                    results[k]['one_stage_frac_rewards_GA'] = OL.frac_rewards
                    results[k]['one_stage_int_rewards_GA'] = OL.int_rewards
                    results[k]['frac_sol_one_stage_GA'] = OL.xs[-1]
                    results[k]['int_sol_one_stage_GA'] = OL.xs_int[-1]
                    results[k]['eta_one_stage_GA'] = eta
        ...
        if 'FTRL-l2' in algorithms:
            ...
            for eta in etas:
                ...
                if sum(OL.int_rewards) > sum(rewards):
                    rewards = OL.int_rewards
                    results[k]['frac_rewards_FTRL_l2'] = OL.frac_rewards
                    results[k]['int_rewards_FTRL_l2'] = OL.int_rewards
                    results[k]['frac_sol_FTRL_l2'] = OL.xs[-1]
                    results[k]['int_sol_FTRL_l2'] = OL.xs_int[-1]
                    results[k]['eta_FTRL_l2'] = eta
        ...
        if 'FTRL-entropy' in algorithms:
            ...
            for eta in etas:
                ...
                if sum(OL.int_rewards) > sum(rewards):
                    rewards = OL.int_rewards
                    results[k]['frac_rewards_FTRL_entropy'] = OL.frac_rewards
                    results[k]['int_rewards_FTRL_entropy'] = OL.int_rewards
                    results[k]['frac_sol_FTRL_entropy'] = OL.xs[-1]
                    results[k]['int_sol_FTRL_entropy'] = OL.xs_int[-1]
                    results[k]['eta_FTRL_entropy'] = eta
        ...
        if 'Random' in algorithms:
            ...
            rewards /= iterations
            results[k]['random'] = randomAlg.solve()
        ...
        if 'OPT' in algorithms:
            ...
            results[k]['frac_opt'] = fracVal
            results[k]['int_opt'] = intVal
            results[k]['frac_sol'] = fracSol
            results[k]['int_sol'] = intSol
        ...
        if 'balkanski' in algorithms:
            ...
            if not 'OPT' in algorithms:
                ...
                results[k][f'balkanski_val'] = balkanskiVal
                results[k]['balkanski_sol'] = sol
        ...
        if 'repGreedy' in algorithms:
            ...
            results[k][f'repGreedy_val'] = repGreedyVal
            results[k]['repGreedy_sol'] = repGreedySol

    with open(f'./results/{dataset}_tradeoff.pkl', 'wb') as file:
